...
Ciphertext SchemeAlgo::powerOf2(Ciphertext& cipher, const long precisionBits, const long logDegree) {
	Ciphertext res = cipher;
	for (long i = 0; i < logDegree; ++i) {
		scheme.squareAndEqual(res);
		scheme.reScaleByAndEqual(res, precisionBits);
	}
	return res;
}

Ciphertext* SchemeAlgo::powerOf2Extended(Ciphertext& cipher, const long precisionBits, const long logDegree) {
...
Ciphertext SchemeAlgo::power(Ciphertext& cipher, const long precisionBits, const long degree) {
	long logDegree = log2((double)degree);
	long po2Degree = 1 << logDegree;

	Ciphertext res = powerOf2(cipher, precisionBits, logDegree);
	long remDegree = degree - po2Degree;
	if(remDegree > 0) {
		Ciphertext tmp = power(cipher, precisionBits, remDegree);
...
Ciphertext* SchemeAlgo::powerExtended(Ciphertext& cipher, const long precisionBits, const long degree) {
	Ciphertext* res = new Ciphertext[degree];
	long logDegree = log2((double)degree);
	Ciphertext* cpows = powerOf2Extended(cipher, precisionBits, logDegree);
	long idx = 0;
	for (long i = 0; i < logDegree; ++i) {
		long powi = (1 << i);
		res[idx++] = cpows[i];
		for (int j = 0; j < powi-1; ++j) {
			...
			scheme.reScaleByAndEqual(res[idx++], precisionBits);
		}
	}
...
Ciphertext SchemeAlgo::prodOfPo2(Ciphertext*& ciphers, const long precisionBits, const long logDegree) {
	...
	for (long i = logDegree - 1; i >= 0; --i) {
		...
		for (long j = first; j < last; ++j) {
			...
			scheme.reScaleByAndEqual(tmp[j], precisionBits);
		}
		NTL_EXEC_RANGE_END;
		res = tmp;
	}
...
Ciphertext SchemeAlgo::prod(Ciphertext*& ciphers, const long precisionBits, const long degree) {
	...
	for (long i = 0; i < logDegree; ++i) {
		if(bit(degree, i)) {
			...
			Ciphertext iprod = prodOfPo2(tmp, precisionBits, i);
			if(isinit) {
...
Ciphertext SchemeAlgo::distance(Ciphertext& cipher1, Ciphertext& cipher2, const long precisionBits) {
	Ciphertext cres = scheme.sub(cipher1, cipher2);
	scheme.squareAndEqual(cres);
	scheme.reScaleByAndEqual(cres, precisionBits);
...
Ciphertext SchemeAlgo::innerProd(Ciphertext*& ciphers1, Ciphertext*& ciphers2, const long precisionBits, const long size) {
	...
	scheme.reScaleByAndEqual(cip, precisionBits);
	return cip;
}
...
Ciphertext SchemeAlgo::inverse(Ciphertext& cipher, const long precisionBits, const long steps) {
	ZZ precision = power2_ZZ(precisionBits);
	Ciphertext cpow = cipher;
	Ciphertext tmp = scheme.addConst(cipher, precision);
	scheme.modDownByAndEqual(tmp, precisionBits);
	Ciphertext res = tmp;

	for (long i = 1; i < steps; ++i) {
		scheme.squareAndEqual(cpow);
		scheme.reScaleByAndEqual(cpow, precisionBits);
		tmp = cpow;
		scheme.addConstAndEqual(tmp, precision);
		scheme.multAndEqual(tmp, res);
		scheme.reScaleByAndEqual(tmp, precisionBits);
		res = tmp;
	}
	return res;
}

Ciphertext* SchemeAlgo::inverseExtended(Ciphertext& cipher, const long precisionBits, const long steps) {
	ZZ precision = power2_ZZ(precisionBits);
...
Ciphertext SchemeAlgo::function(Ciphertext& cipher, string& funcName, const long precisionBits, const long degree) {
...
Ciphertext SchemeAlgo::functionLazy(Ciphertext& cipher, string& funcName, const long precisionBits, const long degree) {
	Ciphertext* cpows = powerExtended(cipher, precisionBits, degree);
...
Ciphertext* SchemeAlgo::functionExtended(Ciphertext& cipher, string& funcName, const long precisionBits, const long degree) {
	Ciphertext* cpows = powerExtended(cipher, precisionBits, degree);
